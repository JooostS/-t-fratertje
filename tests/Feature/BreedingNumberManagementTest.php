...
it('registers a breeding number for an active member', function () {
    ...
    $this->post(route('breeding-numbers.store'), ['member_id' => $member->id, 'breeding_number' => 'nbvv-555', 'issue_year' => 2021])
        ->assertRedirect(route('breeding-numbers.index'));

    expect($member->fresh()->nbvv_number)->toBe('NBVV-555');
});

it('refuses a duplicate breeding number', function () {
    BreedingNumber::factory()->create(['breeding_number' => 'NBVV-555']);
    $member = memberWithoutNumber();
...
it('gives a member at most one breeding number', function () {
    ...
    $this->post(route('breeding-numbers.store'), ['member_id' => $member->id, 'breeding_number' => 'NBVV-999', 'issue_year' => 2021])
...
it('refuses to link a breeding number to a guest or inactive member', function () {
    ...
    $this->post(route('breeding-numbers.store'), ['member_id' => $guest->id, 'breeding_number' => 'NBVV-1', 'issue_year' => 2021])
        ->assertSessionHasErrors('member_id');
    $this->post(route('breeding-numbers.store'), ['member_id' => $inactive->id, 'breeding_number' => 'NBVV-2', 'issue_year' => 2021])
        ->assertSessionHasErrors('member_id');
});

it('updates a breeding number and keeps it unique', function () {
    $mine = BreedingNumber::factory()->create(['breeding_number' => 'NBVV-1']);
    BreedingNumber::factory()->create(['breeding_number' => 'NBVV-2']);

    $this->put(route('breeding-numbers.update', $mine), ['breeding_number' => 'NBVV-1', 'issue_year' => 2019])
        ->assertSessionHasNoErrors();
    $this->put(route('breeding-numbers.update', $mine), ['breeding_number' => 'NBVV-2', 'issue_year' => 2019])
        ->assertSessionHasErrors('breeding_number');
});
